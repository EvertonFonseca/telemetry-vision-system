...
# app/dao/chat_dao.R
# Integração do chat com OpenAI + MariaDB + geração de gráfico dinâmico
# Everton Fonseca — versão com melhorias (pivot, TZ, JSON robusto, df_ctx no retorno, ggplot binder enxuto)
...
loadNamesGgplot <- function(env) {
  bind <- function(name) {
    if (exists(name, envir = asNamespace("ggplot2"), inherits = FALSE)) {
      env[[name]] <- get(name, envir = asNamespace("ggplot2"))
    }
  }
  for (nm in c(
    "ggplot","aes","geom_line","geom_bar","geom_point","geom_col","geom_text",
    "labs","xlab","ylab","theme_minimal","theme_classic","theme_bw",
    "scale_y_continuous","scale_x_continuous","facet_wrap","facet_grid",
    "geom_histogram","geom_density","geom_area","geom_step","geom_vline","geom_hline",
    "scale_color_manual","scale_fill_manual","coord_flip","coord_cartesian",
    "guides","guide_legend","element_text","theme","ggtitle"
  )) bind(nm)
  env
}
...
# =========================================================
# Avalia código de plot retornado usando df_ctx expandido
# + fallback automático caso o código quebre
# =========================================================
safe_eval_plot <- function(plot_code, df_ctx) {
  # 1) expande JSON para wide
  df_ctx <- expand_data_oc(df_ctx)
...
handle_user_message <- function(user_msg, ctx = NULL) {
  ...
  list(
    items = items,
    ctx   = new_ctx,
    df_ctx = df  # <- ESSENCIAL para exportar/gerar PDF fora do Rmd
  )
}
