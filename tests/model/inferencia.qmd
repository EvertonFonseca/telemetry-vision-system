...
```{r}
frames <- get_frames_window(con,490566,janela = 40)
...
```{r}
codigo <- 465672
anime <- build_gif_from_blob( get_frames_window(con,codigo,janela = 30))
print(anime)
```
...
frames <- get_frames_from_to(con,from_time = from,to_time = to,camera_id = c(1,2))
build_gif_from_blob(frames)

estrutura <- readRDS("estrutura.rds")
